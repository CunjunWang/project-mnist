...
SERVER_CLOSED_CONNECTION_ERROR = "Connection closed by database."
    ...
    def _read_from_socket(self, length=None):
        ...
        try:
            while True:
                ...
                # an empty string indicates the database shutdown the socket
...
class Connection(object):
    "Manages TCP communication to and from a Redis database"
    ...
    def connect(self):
        "Connects to the Redis database if not already connected"
        if self._sock:
            return
        try:
            sock = self._connect()
        except socket.timeout:
            raise TimeoutError("Timeout connecting to database")
        except socket.error:
            e = sys.exc_info()[1]
    ...
    def send_packed_command(self, command):
        "Send an already packed command to the Redis database"
        if not self._sock:
            self.connect()
        try:
    ...
    def send_command(self, *args):
        "Pack and send a command to the Redis database"
    ...
    def pack_command(self, *args):
        ...
        # the command name, e.g., 'CONFIG GET'. The Redis database expects these
