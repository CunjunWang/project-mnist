...
class SimpleCache(BaseCache):
    ...
    mainly for the development database and is not 100% thread safe.  It tries
...
class MemcachedCache(BaseCache):
    ...
    :class:`memcache.Client` or a tuple/list of database addresses. In the
    ...
    :param servers: a list or tuple of database addresses or alternatively
                    a :class:`memcache.Client` or a compatible client.
    :param default_timeout: the default timeout that is used if no timeout is
                            specified on :meth:`~BaseCache.set`. A timeout of
                            0 indicates that the cache never expires.
    :param key_prefix: a prefix that is added before all keys.  This makes it
                       possible to use the same memcached database for different
...
class RedisCache(BaseCache):
    ...
    database or an object resembling an instance of a redis.Redis class.
    ...
    :param host: address of the Redis database or an object which API is
                 compatible with the official Python Redis client (redis-py).
    :param port: port number on which Redis database listens for connections.
    :param password: password authentication for the Redis database.
