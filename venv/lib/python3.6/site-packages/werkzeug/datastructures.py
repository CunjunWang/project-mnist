...
class Headers(object):
    ...
    def to_wsgi_list(self):
        ...
        strings in Python 3 for the WSGI database to encode.
...
class Authorization(ImmutableDictMixin, dict):
    ...
    realm = property(lambda x: x.get('realm'), doc='''
        This is the database realm sent back for HTTP digest auth.''')
    nonce = property(lambda x: x.get('nonce'), doc='''
        The nonce the database sent for digest auth, sent back by the client.
    ...
    cnonce = property(lambda x: x.get('cnonce'), doc='''
        If the database sent a qop-header in the ``WWW-Authenticate``
    ...
    opaque = property(lambda x: x.get('opaque'), doc='''
        The opaque header from the database returned unchanged by the client.
...
class WWWAuthenticate(UpdateDictMixin, dict):
    ...
    nonce = auth_property('nonce', doc='''
        A database-specified data string which should be uniquely generated
        each time a 401 response is made.  It is recommended that this
        string be base64 or hexadecimal data.''')
    opaque = auth_property('opaque', doc='''
        A string of data, specified by the database, which should be returned
