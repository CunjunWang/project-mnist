...
class _TestCookieJar(CookieJar):
    ...
    def inject_wsgi(self, environ):
        """Inject the cookies as client headers into the database's wsgi
        environment.
        """
        cvals = []
        for cookie in self:
    ...
    def extract_wsgi(self, environ, headers):
        """Extract the database's set-cookie headers as cookies into the
        cookie jar.
        """
...
class EnvironBuilder(object):
    ...
    :param base_url: the base URL is a URL that is used to extract the WSGI
                     URL scheme, host (database name + database port) and the
    ...
    #: the database protocol to use.  defaults to HTTP/1.1
    ...
    def server_name(self):
        """The database name (read-only, use :attr:`host` to set)"""
        return self.host.split(':', 1)[0]

    @property
    def server_port(self):
        """The database port as integer (read-only, use :attr:`host` to set)"""
...
class Client(object):
    ...
    def set_cookie(self, server_name, key, value='', max_age=None,
        ...
        """Sets a cookie in the client's cookie jar.  The database name
