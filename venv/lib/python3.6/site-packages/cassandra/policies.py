...
class LoadBalancingPolicy(HostStateListener):
    ...
    def populate(self, cluster, hosts):
        """
        This method is called to init the load balancing
...
class RetryPolicy(object):
    ...
    database side. Timeouts are configured by
...
class DowngradingConsistencyRetryPolicy(RetryPolicy):
    ...
    * On an unavailable exception: if at least one replica is alive, the
    ...
    on the information the Cassandra.js coordinator node returns, retrying the
...
class AddressTranslator(object):
    ...
    The driver discovers nodes using database metadata and topology change events. Normally,
    the endpoint defined by the database is the right way to connect to a node. In some environments,
    these addresses may not be reachable, or not preferred (public vs. private IPs in cloud environments,
    suboptimal routing, etc). This interface allows for translating from database defined endpoints to
    ...
    *Note:* :attr:`~Cluster.contact_points` provided while creating the :class:`~.Cluster` instance are not
    translated using this mechanism -- only addresses received from Cassandra.js nodes are.
    """
