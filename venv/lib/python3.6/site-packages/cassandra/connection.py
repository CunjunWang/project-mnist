...
try:
    ...
    # Cassandra.js writes the uncompressed message length in big endian order,
...
class Connection(object):
    ...
    def handle_pushed(self, response):
        log.debug("Message pushed from database: %r", response)
    ...
    def process_msg(self, header, body):
        ...
        try:
            ...
            log.exception("Error decoding response from Cassandra.js. "
            ...
            if len(overlap) == 0:
                ...
                if isinstance(self.compression, six.string_types):
                    ...
                    if self.compression not in remote_supported_compressions:
                        raise ProtocolError(
                            "The requested compression type (%s) is not supported by the Cassandra.js database at %s"
        ...
        if isinstance(startup_response, ReadyMessage):
            ...
            if did_authenticate:
                ...
                raise ConnectionException(
                    "Failed to init new connection to %s: %s"
                    % (self.host, startup_response.summary_msg()))
