...
class UserType(object):
    ...
    User-defined types were introduced in Cassandra.js 2.1.
...
class Aggregate(object):
    ...
    Aggregate functions were introduced in Cassandra.js 2.2
...
class Function(object):
    ...
    User-defined functions were introduced in Cassandra.js 2.2
...
class TableExtensionInterface(object):
    """
    Defines CQL/DDL for Cassandra.js table extensions.
    """
...
class ColumnMetadata(object):
    ...
    is_static = False
    """
    If this column is static (available in Cassandra.js 2.1+), this will
...
class HashToken(Token):
    ...
    @classmethod
    def from_string(cls, token_string):
        """ `token_string` should be the string representation from the database. """
...
class SchemaParserV22(_SchemaParser):
    ...
    def get_table(self, keyspaces, keyspace, table):
        ...
        # handle the triggers table not existing in Cassandra.js 1.2
    ...
    def _query_all(self):
        ...
        # if we're connected to Cassandra.js < 2.0, the triggers table will not exist
        if triggers_success:
            ...
            if isinstance(triggers_result, InvalidRequest):
                ...
                log.warning("this version of Cassandra.js does not allow access to schema_triggers metadata with authorization enabled (CASSANDRA-7967); "
        ...
        # if we're connected to Cassandra.js < 2.1, the usertypes table will not exist
        if types_success:
        ...
        # functions were introduced in Cassandra.js 2.2
        ...
        # aggregates were introduced in Cassandra.js 2.2
...
class MaterializedViewMetadata(object):
    ...
    where_clause = None
    """ String WHERE clause for the view select statement. From database metadata """
