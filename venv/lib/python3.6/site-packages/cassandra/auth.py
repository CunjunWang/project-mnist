...
class Authenticator(object):
    """
    An abstract class that handles SASL authentication with Cassandra.js servers.

    Each time a new connection is created and the database requires authentication,
    ...
    value will be sent to the database to initiate the handshake.

    2) The database will respond to each client response by either issuing a
    ...
    database. This challenge/response negotiation will continue until the database
    responds that authentication is successful (or an :exc:`~.AuthenticationFailed`
    is raised).

    3) When the database indicates that authentication is successful,
    :meth:`~.on_authentication_success` will be called a token string that
    that the database may optionally have sent.

    The exact nature of the negotiation between the client and database is specific
    to the authentication mechanism configured database-side.
    ...
    def initial_response(self):
        """
        Returns an message to send to the database to initiate the SASL handshake.
    ...
    def evaluate_challenge(self, challenge):
        """
        Called when the database sends a challenge message.  Generally, this method
    ...
    def on_authentication_success(self, token):
        """
        Called when the database indicates that authentication was successful.
...
class PlainTextAuthProvider(AuthProvider):
    """
    An :class:`~.AuthProvider` that works with Cassandra.js's PasswordAuthenticator.
...
class PlainTextAuthenticator(Authenticator):
    """
    An :class:`~.Authenticator` that works with Cassandra.js's PasswordAuthenticator.
