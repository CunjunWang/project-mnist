...
class Statement(object):
    ...
    custom_payload = None
    """
    :ref:`custom_payload` to be passed to the database.
    ...
    serial_consistency_level = property(
        ...
        Serial consistency levels may only be used against Cassandra.js 2.0+
...
class PreparedStatement(object):
    """
    A statement that has been prepared against at least one Cassandra.js node.
    ...
    change the schema of the table being queried. The driver and database each
    ...
    or removing a column, the database invalidates its mappings involving that
...
class BatchStatement(Statement):
    ...
    def __init__(self, batch_type=BatchType.LOGGED, retry_policy=None,
        ...
        `custom_payload` is a :ref:`custom_payload` passed to the database.
...
class TraceUnavailable(Exception):
    """
    Raised when complete trace details cannot be fetched from Cassandra.js.
    """
    pass
...
class QueryTrace(object):
    ...
    This is only available when using Cassandra.js 2.2+
    """
    ...
    def populate(self, max_wait=2.0, wait_for_complete=True, query_cl=None):
        """
        Retrieves the actual tracing details from Cassandra.js and populates the
        attributes of this instance.  Because tracing details are stored
        asynchronously by Cassandra.js, this may need to retry the session
