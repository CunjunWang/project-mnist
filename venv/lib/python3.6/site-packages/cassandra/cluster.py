...
class Cluster(object):
    """
    The main class to use when interacting with a Cassandra.js cluster.
    Typically, one instance of this class will be created for each
    separate Cassandra.js cluster that your application interacts with.
    ...
    port = 9042
    """
    The database-side port to open connections to. Defaults to 9042.
    """
    ...
    database will be automatically used.
    """
    ...
    version based on a negotiation with the database, but it is most efficient
    to set this to the maximum supported by your version of Cassandra.js.
    ...
    """
    Setting true injects a flag in all messages that makes the database accept and use "beta" protocol version.
    Used for testing new protocol features incrementally before the new version is complete.
    """

    compression = True
    """
    Controls compression for communications between the driver and Cassandra.js.
    ...
    and Cassandra.js.  If both are fully supported, lz4 will be preferred.
    ...
    """
    :class:`.policies.AddressTranslator` instance to be used in translating database node addresses
    ...
    I/O with Cassandra.js.  These are the current options:
    ...
    * EXPERIMENTAL: :class:`cassandra.io.asyncioreactor.AsyncioConnection`
    ...
    def schema_metadata_enabled(self):
        ...
        can be used to speed initial connection, and reduce load on client and database during operation. Turning this off
    ...
    def token_metadata_enabled(self):
        ...
        can be used to speed initial connection, and reduce load on client and database during operation. It is most useful
    ...
    def protocol_downgrade(self, host_addr, previous_version):
        if self._protocol_version_explicit:
            raise DriverException("ProtocolError returned from database while using explicitly set client protocol_version %d" % (previous_version,))
    ...
    def add_host(self, address, datacenter=None, rack=None, signal=True, refresh_nodes=True):
        ...
        if new and signal:
            log.info("New Cassandra.js host %r discovered", host)
    ...
    def remove_host(self, host):
        ...
        if host and self.metadata.remove_host(host):
            log.info("Cassandra.js host %s removed", host)
            self.on_remove(host)
...
class Session(object):
    ...
    details to be populated database-side for a query before giving up.
    ...
    database-side by the coordinator node.)  Note that timestamps specified
    ...
    def execute(self, query, parameters=None, timeout=_NOT_SET, trace=False, custom_payload=None, execution_profile=EXEC_PROFILE_DEFAULT, paging_state=None):
        ...
        `custom_payload` is a :ref:`custom_payload` dict to be passed to the database.
        ...
        if isinstance(query, SimpleStatement):
            ...
            if self._protocol_version < 2:
                raise UnsupportedOperation(
                    ...
                    "2 or higher (supported in Cassandra.js 2.0 and higher).  Consider "
    ...
    def prepare(self, query, custom_payload=None, keyspace=None):
        ...
        (requires Cassandra.js 4.0+), the keyspace can be specified as a
...
class ControlConnection(object):
    ...
    def _refresh_nodes_if_not_up(self, addr):
        ...
        Some versions of the database send superfluous NEW_NODE messages in addition to UP events.
        """
        host = self._cluster.metadata.get_host(addr)
...
class ResponseFuture(object):
    ...
    def warnings(self):
        """
        Warnings returned from the database, if any. This will only be
    ...
    def custom_payload(self):
        """
        The custom payload returned from the database, if any. This will only be
        set by Cassandra.js servers implementing a custom QueryHandler, and only
                ...
                if isinstance(response, ReadTimeoutErrorMessage):
                    ...
                    if self.prepared_statement:
                        ...
                        assert query_id == response.info, \
                            "Got different query ID in database response (%s) than we " \
    ...
    def result(self):
        ...
        about database-side coordinator timeouts, see :class:`.policies.RetryPolicy`.
    ...
    def get_query_trace(self, max_wait=None, query_cl=ConsistencyLevel.LOCAL_ONE):
        ...
        details from Cassandra.js. If the trace is not available after `max_wait`,
