...
class Host(object):
    """
    Represents a single Cassandra.js node.
    """
    ...
    This is not present in the ``system.local`` table for older versions of Cassandra.js. It is also not queried if
    ...
    versions of Cassandra.js. It is also not queried if :attr:`~.Cluster.token_metadata_enabled` is ``False``.
