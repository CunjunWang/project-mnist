...
"""
Representation of Cassandra.js data types. These classes should make it simple for
the library (and caller software) to deal with Cassandra.js-style Java class type
...
the corresponding CQL or Cassandra.js type strings.
"""
...
def lookup_casstype_simple(casstype):
    """
    Given a Cassandra.js type name (either fully distinguished or not), hand
...
def lookup_casstype(casstype):
    """
    Given a Cassandra.js type as a string (possibly including parameters), hand
    back the CassandraType class responsible for it. If a name is not
    recognized, a custom _UnrecognizedType subclass will be created for it.
...
class _CassandraType(object):
    ...
    def cass_parameterized_type_with(cls, subtypes, full=False):
        """
        Return the name of this type as it would be expressed by Cassandra.js,
        ...
        string includes the Cassandra.js names for those subclasses as well,
    ...
    def cass_parameterized_type(cls, full=False):
        """
        Return a Cassandra.js type specifier for this type. If this type has
...
class ColumnToCollectionType(_ParameterizedType):
    ...
    Cassandra.js includes this. We don't actually need or want the extra
    information.
    """
...
def cql_typename(casstypename):
    """
    Translate a Cassandra.js-style type specifier (optionally-fully-distinguished
